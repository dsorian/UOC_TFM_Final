Shader "Custom/RandomizedDynamicWaveShader"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // Genera un valor pseudoaleatorio basado en coordenadas y tiempo
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                // Se agrega _Time.y para hacer que cambien dinámicamente las zonas activas
                float randomValue = rand(sectionCoords + float2(_Time.y, _Time.y));

                // Modificar la velocidad de las ondas según la posición de la sección
                float positionBasedSpeed = _WaveSpeed + rand(sectionCoords) * 0.5;

                // Aplicar movimiento solo si el valor aleatorio está por debajo de la probabilidad configurada
                float2 distortedUV = i.uv;
                if (randomValue < _MoveProbability)
                {
                    distortedUV += applyWave(i.uv, sectionCenter, time * positionBasedSpeed, _WaveStrength);
                }
