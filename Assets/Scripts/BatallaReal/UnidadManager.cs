...
public class UnidadManager : MonoBehaviour
{
   // public NavMeshAgent miNavMeshAgent;
    ...
    public bool idle = true, andando = false, atacando = false, defendiendo = false, cargandoCatapulta = false;
        ...
        */
        if(player == "Player1"){
            ...
            case 2: //Caballería sí puede atacar y mover
                ...
                if( horizontalInputP1 != 0 || verticalInputP1 !=0){
                    Debug.Log("prueba: movimiento: "+horizontalInputP1+"-"+verticalInputP1);
                    //Poner el movimiento con una sola línea:
                    //transform.Translate(new Vector3(horizontalInputP1, verticalInputP1, 0) * walkSpeed * Time.deltaTime,Space.Self);
...
//                    else{
                        ...
                        if(andando && !atacando){
                            MoverUnidades();
                            //elBatallaManager.elSoundManager.PlayMusic(elBatallaManager.elSoundManager.musicaUnidadSeleccionada[2],true,0.5f,"UnidadSeleccionadaP1Source");
                        }
//                    }
                }
                break;
            ...
            if( !atacando && !cargandoCatapulta && !defendiendo && tipoUnidad != 2 ){
                ...
                //transform.Translate(new Vector3(horizontalInput, verticalInput, 0) * moveSpeed * Time.deltaTime);
                    transform.Translate(Vector3.forward * Time.deltaTime * verticalInputP1 * walkSpeed);
                    if( tipoUnidad != 0 )
                        transform.Translate(Vector3.right * Time.deltaTime * horizontalInputP1 * walkSpeed);
                    MoverUnidades();
                }
            }
        }
    ...
    public void CrearUnidades(GameObject laUnidad,int num,Material elMaterial1,Material elMaterial2,int numPlayer,bool seleccionada, int tipoUnidad){
        ...
        for( int i=0; i<numUnidadesCombatiendo;i++){
            ...
            if( tipoUnidad == 0)
                ...
                anguloRotacion = 90;
            unidades[i] = Instantiate(modeloUnidad,new Vector3(transform.GetChild(i).position.x, transform.GetChild(i).position.y, transform.GetChild(i).position.z), Quaternion.Euler(0,anguloRotacion,0));
if(tipoUnidad == 2 && player == "Player2")
    ...
    public void GolpearCaballeria(){
        for(int i=0; i<unidades.Length;i++){
            if(unidades[i] != null){
                switch(tipoUnidad){
                case 2: 
                    unidades[i].GetComponent<Unidad>().GolpearCaballeria();
                    break;
                default:
                    Debug.LogError("UnidadManager: Caso no contemplado de GolpearCaballeria()");
                    break;
                }
            }
        }
...
*/
    ...
    public bool AlgunaUnidadTieneObjetivoCerca(){
        ...
        for( int i=0; i<unidades.Length; i++){
            if( unidades[i] != null && unidades[i].GetComponent<Unidad>().DistanciaToObjetivo() <= 5.0f){
